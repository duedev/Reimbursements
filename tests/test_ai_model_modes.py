...
def client(tmp_path, monkeypatch):
    ...
    saved = (pr.AUTOROTATE_ENABLED, pr.AUTOCROP_ENABLED, pr.GRAYSCALE_ENABLED,
             pr.AUTOCROP_AGGRESSIVENESS)
    with TestClient(server.app) as c:
        yield c
    (pr.AUTOROTATE_ENABLED, pr.AUTOCROP_ENABLED, pr.GRAYSCALE_ENABLED,
     pr.AUTOCROP_AGGRESSIVENESS) = saved
...
def test_processing_preset_camscanner_skips_autocrop(client):
    d = client.post("/settings/processing/preset", json={"preset": "camscanner"}).json()
    assert d["ok"] is True
    assert d["preset"] == "camscanner"
    assert d["autocrop"] is False           # scan apps already crop
    ...
    assert proc["autocrop"] is False


def test_processing_preset_scanned_alias(client):
    d = client.post("/settings/processing/preset", json={"preset": "scanned"}).json()
    assert d["ok"] is True and d["autocrop"] is False


def test_processing_preset_photo_enables_autocrop(client):
    d = client.post("/settings/processing/preset", json={"preset": "photo"}).json()
    assert d["autocrop"] is True
    assert d["autocrop_aggressiveness"] == 85
    assert pr.AUTOCROP_ENABLED is True
