...
def test_steps_lm_ocr_fails_paddle_fallback(tmp_path, monkeypatch):
    """LM Studio OCR fails → PaddleOCR fallback; step log shows the handoff."""
    img = _setup(monkeypatch, tmp_path,
    ...
    assert by_step["paddle_ocr"]["ok"] is True
    assert "fallback" in by_step["paddle_ocr"]["detail"].lower()
...
def test_steps_fully_failed_all_logged(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pr, "_extract_paddle_ocr",   MagicMock(return_value=None))
    steps: list = []
    data = pr._extract_receipt_with_status(MagicMock(), img, None, steps)
    assert data is None
    by_step = {s["step"]: s for s in steps}
    assert by_step["vision"]["ok"] is False
    assert by_step["paddle_ocr"]["ok"] is False
