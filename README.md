...
| **Chevron/Texaco Rewards export** | The bundled `chevron_receipt_downloader.user.js` userscript exports your Rewards Wallet purchase receipts as one PDF (one receipt per page, discount duplicates skipped). Drop it on Add Receipts — each page becomes a receipt, and because the pages are digital text the app reads the **PDF text layer directly** (no OCR errors, no AI-OCR cost). See `CHEVRON_RECEIPTS.md` |
